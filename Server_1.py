    ...
    while True:
        ...
        if (recibido == 'get'):
            ...
            for i in range(10):
                ...
                arrayCheckSum[i] = reduce(
                    lambda x, y: x + y, map(ord, array[i]))
            print("")
            print("Se ha Guardado toda la traza correctamente, se procedera a enviar")
            for i in range(10):
                data = conn.recv(1024)  # recibir datos desde el enlace
                # Decodificar respuestas del cliente
                recibido = data.decode("utf-8")
                while (recibido != 'ack'):
                    data2 = "..Dato Rechazado.."
                    response = data2.encode("utf-8")
                    conn.send(response)
                    print("Error en ack!!")
                    data = conn.recv(1024)  # recibir datos desde el enlace
                    # Decodificar respuestas del clientes
                    recibido = data.decode("utf-8")
                cadena = arrayTokens[i] + ',' + \
                    array[i] + ',' + str(arrayCheckSum[i])
